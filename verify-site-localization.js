...
assert(/\.reader-page \.header-content\s*\{[\s\S]*?min-height:\s*var\(--reader-header-height\);[\s\S]*?grid-template-rows:\s*auto auto;/.test(readerStyle),
    'Reader header content must use the shared height and two-row layout.');
assert(/\.reader-page \.reader-language-control\s*\{[\s\S]*?grid-column:\s*1 \/ -1;[\s\S]*?grid-row:\s*1;[\s\S]*?justify-self:\s*center;/.test(readerStyle),
    'Reader language control must occupy the centered top row.');
